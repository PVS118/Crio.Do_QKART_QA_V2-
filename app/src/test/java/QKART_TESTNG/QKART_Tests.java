...
public class QKART_Tests {
     ...
     * boolean status;
     * 
     * // Visit the home page Home homePage = new Home(driver); homePage.navigateToHome();
     * 
     * // SLEEP_STMT_01 : Wait for Page to Load // Thread.sleep(5000);
     * 
     * // Search for the "yonex" product status = homePage.searchForProduct("yonex"); if (!status) {
     * logStatus("TestCase 3", "Test Case Failure. Unable to search for given product", "FAIL");
     * return false; }
     * 
     * // Fetch the search results List<WebElement> searchResults = homePage.getSearchResults();
     * 
     * // Verify the search results are available if (searchResults.size() == 0) {
     * logStatus("TestCase 3",
     * "Test Case Failure. There were no results for the given search string", "FAIL"); return
     * false; }
     * 
     * for (WebElement webElement : searchResults) { // Create a SearchResult object from the parent
     * element SearchResult resultelement = new SearchResult(webElement);
     * 
     * // Verify that all results contain the searched text String elementText =
     * resultelement.getTitleofResult(); if (!elementText.toUpperCase().contains("YONEX")) {
     * logStatus("TestCase 3", "Test Case Failure. Test Results contains un-expected values: " +
     * elementText, "FAIL"); return false; } }
     * 
     * logStatus("Step Success", "Successfully validated the search results ", "PASS"); //
     * SLEEP_STMT_02 Thread.sleep(2000);
     * 
     * // Search for product status = homePage.searchForProduct("Gesundheit"); if (!status) {
     * logStatus("TestCase 3", "Test Case Failure. Unable to search for given product", "FAIL");
     * return false; }
     * 
     * // Verify no search results are found searchResults = homePage.getSearchResults(); if
     * (searchResults.size() == 0) { if (homePage.isNoResultFound()) { logStatus("Step Success",
     * "Successfully validated that no products found message is displayed", "PASS"); }
     * logStatus("TestCase 3",
     * "Test Case PASS. Verified that no search results were found for the given text", "PASS"); }
     * else { logStatus("TestCase 3",
     * "Test Case Fail. Expected: no results , actual: Results were available", "FAIL"); return
     * false; }
     * 
     * return true;
     */
