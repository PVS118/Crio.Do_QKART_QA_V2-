...
public class Home {
    ...
    public Boolean isNoResultFound() {
       ...
       try {
            ...
            if(driver.findElement(By.xpath("//*[@id='root']/div/div/div[3]/div[1]/div[2]/div/h4")).isDisplayed())
            {
            status =true;
            }
            else
    ...
    public Boolean addProductToCart(String productName) {
        try {
             ...
             */
            //driver.findElement(By.className( "css-54wre3" )).click();
           // Thread.sleep(2000);
           driver.findElement(By.xpath("//*[@id='root']/div/div/div[3]/div[1]/div[2]/div/div/div[2]/button")).click();
        Thread.sleep(2000);
    ...
    public Boolean clickCheckout() {
        ...
        try {
            ...
            driver.findElement(By.xpath("//button[@class='MuiButton-root MuiButton-contained MuiButton-containedPrimary MuiButton-sizeMedium MuiButton-containedSizeMedium MuiButtonBase-root checkout-btn css-177pwqq']")).click();
            Thread.sleep(3000);
            return true;
        } catch (Exception e) {
    ...
    //             if(productName.contains(driver.findElement(By.xpath("//*[@class='MuiBox-root css-zgtx0t'])[" + i + "]//*[@class='MuiBox-root css-lgjj37g']/div[1]")).getText())){
    //                 currentQty =Integer.valueOf(cartContents.get(i-1).findElement(By.className("css-olyig7")).getText());
                    
    //                 while(currentQty != quantity){
    //                 if(currentQty < quantity){

    //                     cartContents.get(i -1).findElement(By.tagName("button")).click();
    ...
    for(int i=1; i<=ItemsInCart.size(); i++){
      if(productName.contains(driver.findElement(By.xpath("(//*[@class='MuiBox-root css-zgtx0t'])[" +i+"]//*[@class='MuiBox-root css-1gjj37g']/div[1]")).getText())){
       currentQty = Integer.valueOf(ItemsInCart.get(i-1).findElement(By.className("css-olyig7")).getText());
    // Increment or decrement the quantity of the matching product until the current
    ...
    while(currentQty != quantity){
        if(currentQty < quantity){
            ...
            ItemsInCart.get(i-1).findElements(By.tagName("button")).get(1).click();
            wait.until(ExpectedConditions.textToBePresentInElement(ItemsInCart.get(i-1).findElement(By.className("css-olyig7")),
            String.valueOf(currentQty + 1)));
            Thread.sleep(3000);
        }
        else{
        
            ItemsInCart.get(i-1).findElements(By.tagName("button")).get(0).click();
            Thread.sleep(3000);
            
        }
        currentQty = Integer.valueOf(ItemsInCart.get(i-1).findElement(By.xpath("//div[@data-testid='item-qty']")).getText());
    }
        return true;
